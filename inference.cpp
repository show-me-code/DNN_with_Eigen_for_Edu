...
rMatrixXd inference(model m, rMatrixXd input)
{
    ...
    auto layer8_result = (m.layer8 * layer6_result + m.bias8);
...
int main()
{
    ...
    cout << std::chrono::duration_cast<std::chrono::seconds>(end - start).count() << " second consumed" << endl;
    //cout << result << endl;
}