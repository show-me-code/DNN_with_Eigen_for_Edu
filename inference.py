...
def inference(vec0, vec1, vec2):
    ...
    vec0 = np.reshape(vec0, (-1, 23))
    vec1 = np.reshape(vec1, (-1, 23))
    vec2 = np.reshape(vec2, (-1, 23))
...
if __name__ == '__main__':
    vec0 = np.ones(2300000 * 1)  # generate 100 random samples
    vec1 = np.ones(2300000 * 1)
    vec2 = np.ones(2300000 * 1)
    s = time.time()
    result = inference(vec0, vec1, vec2)
    e = time.time()
    print('time consume', (e - s) / 3)
    # print(result)
